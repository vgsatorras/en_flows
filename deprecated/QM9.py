...
class QM9Dataset(Dataset):
    ...
    def __init__(self, file_address: str, task: str, mode: str='train', 
            transform=None, fully_connected: bool=False, only19=0):
        ...
        self.only19 = only19 # if True, then only use molecules of size 19
    ...
    def load_data(self):
        ...
        if self.only19:
            mask = self.inputs['num_atoms'] == 19
            for k in self.inputs.keys():
                self.inputs[k] = self.inputs[k][mask]
